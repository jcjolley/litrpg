...
.PHONY: build build-lambda build-ui deploy-ui deploy test test-ui test-all test-class clean aws-login stats stats-local list list-local add remove refresh export-prod import-local setup-local-prod localstack localstack-init localstack-down curator curator-local migrate-genres migrate-genres-local help ship ship-deploy worktree-list worktree-sync worktree-cleanup
